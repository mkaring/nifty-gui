...
public class GdxInputSystem implements InputSystem, InputProcessor {
  ...
  public boolean keyUp(final int keyCode) {
    eventQueue.offer(
        GdxKeyboardInputEvent.getInstance(keyCode, (char) 0, false, false, isShiftDown(), isControlDown()));
    return true;
  }

  @Override
  public boolean keyTyped(final char character) {
    eventQueue.offer(GdxKeyboardInputEvent.getInstance(0, character, false, false, isShiftDown(), isControlDown()));
    return true;
  }
  ...
  public boolean touchDragged(final int screenX, final int screenY, final int pointer) {
    if (input.isButtonPressed(Input.Buttons.LEFT)) {
      eventQueue.offer(
          GdxMouseInputEvent.getMouseEvent(screenX, screenY, 0, pointer, Input.Buttons.LEFT, false, false));
    }
    if (input.isButtonPressed(Input.Buttons.MIDDLE)) {
      eventQueue.offer(
          GdxMouseInputEvent.getMouseEvent(screenX, screenY, 0, pointer, Input.Buttons.MIDDLE, false, false));
    }
    if (input.isButtonPressed(Input.Buttons.RIGHT)) {
      eventQueue.offer(
          GdxMouseInputEvent.getMouseEvent(screenX, screenY, 0, pointer, Input.Buttons.RIGHT, false, false));
    }
    return true;
  }
  ...
  public boolean mouseMoved(final int screenX, final int screenY) {
    eventQueue.offer(
        GdxMouseInputEvent.getMouseEvent(screenX, screenY, 0, 0, GdxMouseInputEvent.NO_BUTTON, false, false));
    return true;
  }
  ...
  public boolean scrolled(final int amount) {
    eventQueue.offer(
        GdxMouseInputEvent.getMouseEvent(input.getX(), input.getY(), amount, 0, GdxMouseInputEvent.NO_BUTTON, false,
            false));
    return true;
  }
}
